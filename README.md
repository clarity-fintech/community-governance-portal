...
> Creator **Chandler William Ferguson** · Org [`clarity-fintech/community-governance-portal`](https://github.com/clarity-fintech/community-governance-portal)
...
**Community Governance Portal** is written entirely in **MIS** (Moniversive Invariant Static, `.mis`) — a
...
```bash
git clone https://github.com/clarity-fintech/community-governance-portal
cd community-governance-portal
...
| `bin/misc mis/kernel/CommunityGovernancePortalKernel.mis --check --compact-letters` | Check a single module directly with the compiler. |

Direct compiler invocation (what the Makefile runs under the hood):

```bash
bin/misc mis/kernel/CommunityGovernancePortalKernel.mis --check --compact-letters
...
```
mis/kernel/CommunityGovernancePortalKernel.mis
mis/packs/debate_window.mis
mis/packs/execute_upgrade.mis
mis/packs/proposal_open.mis
mis/packs/quorum_bound.mis
mis/packs/vote_tally.mis
...
mis/commands/CommunityGovernancePortalCommandCatalog.mis
mis/commands/CommunityGovernancePortalCommands.mis
mis/commands/CommunityGovernancePortalNetworkBind.mis
...
`mis/kernel/CommunityGovernancePortalKernel.mis` — the kernel declares the invariants the compiler enforces on every check:
...
// Title: Community Governance Portal Kernel

module CommunityGovernancePortalKernel {
  ...
  invariant domain_bound: domain == community_governance_portal;
  ...
  invariant pack_proposal_open_flag: pack_proposal_open_bound == true;
  invariant pack_debate_window_flag: pack_debate_window_bound == true;
  invariant pack_vote_tally_flag: pack_vote_tally_bound == true;
  invariant pack_quorum_bound_flag: pack_quorum_bound_bound == true;
  invariant pack_execute_upgrade_flag: pack_execute_upgrade_bound == true;
```
...
// Letter-hash + EMBED gates 3..=6 · settlement clrty-1 / chain 1202
// Compile: bin/misc <file>.mis --check --compact-letters
// Title: Community Governance Portal · DebateWindow

module CommunityGovernancePortalDebateWindow {
  ...
  invariant settlement_network_clrty1: settlement_network == clrty_1;
  invariant kernel_is_misc: compiler_kernel == misc;
  invariant pack_debate_window_bound: pack_bound == true;
  invariant catalog_backlink: mis_catalog_bound == true;

  invariant debate_window_021: debate_window_021_bound == true;
  invariant debate_window_022: debate_window_022_bound == true;
  invariant debate_window_023: debate_window_023_bound == true;
  invariant debate_window_024: debate_window_024_bound == true;
  invariant debate_window_025: debate_window_025_bound == true;
  invariant debate_window_026: debate_window_026_bound == true;
```
...
```
CONNECTED clrty-1/1202 community-governance-portal
RPC https://rpc.clarity-fintech.com
MODULES mis/commands/CommunityGovernancePortalCommands.mis
```
...
{
  "ok": true,
  "module": "CommunityGovernancePortalBacklinkIndex",
  "kernel": "misc",
  "invariant_count": 9,
  "outcome_count": 4,
  "typed_letters": 1245,
...
*Reproduce:* `make check` → `"ok": true`, module `CommunityGovernancePortalBacklinkIndex`, 9 invariants, 4 outcomes, 1245 typed letters.
...
- [clarity-fintech/community-governance-portal](https://github.com/clarity-fintech/community-governance-portal) — this repository
