...
check:
	...
	$(MISC) clrty/community-governance-portal.mis --check --compact-letters
	$(MISC) mis/clrty1_e2e_bind.mis --check --compact-letters
	$(MISC) mis/kernel/CommunityGovernancePortalKernel.mis --check --compact-letters
	@find mis -name '*.mis' | while read f; do $(MISC) "$$f" --check --compact-letters || exit 1; done
	@echo OK community-governance-portal
...
network-connect: commands-check
	...
	@echo MODULES mis/commands/CommunityGovernancePortalCommands.mis
